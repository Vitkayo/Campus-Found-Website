<header class="cf-topbar">
    <div class="cf-container cf-nav">
        ...
        <div class="cf-nav-right">
            ...
            <div class="cf-mobile-shortcuts">
                <a href="{{ route('report.create') }}" class="cf-mobile-report" aria-label="Create report">
                    <i class="bi bi-plus-lg" aria-hidden="true"></i>
                </a>
                <details class="cf-mobile-account-menu">
                    <summary aria-label="Open account menu" title="Account">
                        <i class="bi bi-person-fill" aria-hidden="true"></i>
                    </summary>
                    <div class="cf-mobile-account-panel">
                        @auth
                            <div class="cf-account-name cf-account-name-mobile">
                                <strong>{{ auth()->user()->name }}</strong>
                                <small>{{ auth()->user()->email }}</small>
                            </div>
                            <a href="{{ route('account.show') }}" class="cf-mobile-account-link" data-menu-close><i class="bi bi-speedometer2"></i> My Dashboard</a>
                            <a href="{{ route('account.show') }}#my-reports" class="cf-mobile-account-link" data-menu-close><i class="bi bi-card-list"></i> My Reports</a>
                            <a href="{{ route('account.show') }}#my-claims" class="cf-mobile-account-link" data-menu-close><i class="bi bi-check2-circle"></i> My Claims</a>
                            <form method="post" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="cf-mobile-account-button"><i class="bi bi-box-arrow-right"></i> Log Out</button>
                            </form>
                        @else
                            <a href="{{ route('login') }}" class="cf-mobile-account-link" data-menu-close><i class="bi bi-box-arrow-in-right"></i> Log In</a>
                            <a href="{{ route('register') }}" class="cf-mobile-account-link" data-menu-close><i class="bi bi-person-plus"></i> Create Account</a>
                        @endauth
                    </div>
                </details>
        ...
        <div class="cf-nav-dropdown" id="cf-mobile-menu">
            <nav class="cf-mobile-nav-links" aria-label="Mobile navigation">
                <a href="{{ route('home') }}" class="{{ request()->routeIs('home') ? 'active' : '' }}" data-menu-close><i class="bi bi-house-door" aria-hidden="true"></i><span>Home</span></a>
                <a href="{{ route('board.index') }}" class="{{ request()->routeIs('board.index') ? 'active' : '' }}" data-menu-close><i class="bi bi-grid" aria-hidden="true"></i><span>Board</span></a>
                <a href="{{ route('claims.index') }}" class="{{ request()->routeIs('claims.*') ? 'active' : '' }}" data-menu-close><i class="bi bi-check2-circle" aria-hidden="true"></i><span>Claims</span></a>
                <a href="{{ request()->routeIs('home') ? '#how-it-works' : route('home').'#how-it-works' }}" data-menu-close><i class="bi bi-info-circle" aria-hidden="true"></i><span>About</span></a>
            </nav>

            <div class="cf-mobile-menu-actions">
                @if (auth()->user()?->isAdmin())
                    <a href="{{ route('report.create') }}" class="cf-btn cf-nav-report {{ request()->routeIs('report.*') ? 'active' : '' }}" data-menu-close>
                        <i class="bi bi-plus-lg" aria-hidden="true"></i><span>Report Item</span>
                    </a>
                    <a href="{{ route('admin.dashboard') }}" class="cf-btn cf-btn-outline cf-nav-admin" data-menu-close>Dashboard</a>
                    <form method="post" action="{{ route('admin.logout') }}" class="cf-nav-logout">
                        @csrf
                        <button type="submit" class="cf-btn cf-btn-danger">Logout</button>
                    </form>
